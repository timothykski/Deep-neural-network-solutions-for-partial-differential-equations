...
class CorrelatedBasketOptionPriceCalculator:
    # todo: Note that this is independent from NN training paths
    def __init__(self, correlation_matrix):
        self.correlation_matrix = correlation_matrix
        self.cholesky = np.linalg.cholesky(correlation_matrix)

    # def calculate_call_option_prices(self, X_pred, time_array, K, r, sigma, T, q=0):
    #     rows, cols = X_pred.shape  # Extract the number of time steps and asset paths
    #
    #     option_prices = np.zeros((rows, cols))
    #
    #     for i in range(rows):  # Iterate over time steps
    #         for j in range(cols):  # Iterate over asset paths
    #             # Extract the asset prices for all assets at the current time step for the current path
    #             avg_S = np.mean(X_pred[i, j]) # Shape is (number_of_assets,)
    #
    #             t = time_array[i]  # Current time step
    #             time_to_maturity = T - t  # Time remaining until maturity
    #
    #             if time_to_maturity > 0:
    #                 # Calculate the option price using Monte Carlo simulation
    #                 option_prices[i, j] = self._monte_carlo_price(avg_S, K, time_to_maturity, r, sigma)
    #             else:
    #                 # At maturity, the option price is the payoff
    #                 option_prices[i, j] = max(np.mean(avg_S) - K, 0)
    #
    #     return option_prices
    #
    # def _monte_carlo_price(self, avg_S, K, T, r, sigma):
    #     # Ensure asset_paths is 1D (since now it's a vector of asset prices at the current time step)
    #     # asset_paths = np.atleast_1d(asset_paths)
    #
    #     # Compute the average asset price at the current time step
    #     # avg_asset_price = np.mean(asset_paths)
    #
    #     # Simulate the payoff based on the terminal prices
    ...
    def calculate_call_option_prices(self, X_pred, time_array, K, r, sigma, T, D, mc_paths=20000):
        rows, cols, _ = X_pred.shape  # X_pred shape is (num_paths, num_time_steps, num_assets)
        option_prices = np.zeros(
            (rows, cols))  # Initialize array for option prices with shape (num_paths, num_time_steps)

        for i in range(rows):
            for j in range(cols):
                # Correctly extract the prices at each time step for all assets
                S = X_pred[:, :, :]  # Get the asset prices for all assets over all time steps and paths
                t = time_array[min(j, len(time_array) - 1)]
                time_to_maturity = T - t  # Time to maturity from the current time step
                print(time_to_maturity)

                # For each time step, calculate the option price
                option_prices[i, j] = self._monte_carlo_price(S, K, time_to_maturity, r, D)

        #todo: for numerical methods, there is no meaning to extract intermediate paths

        return option_prices

    def _monte_carlo_price(self, S, K, T, r, D):  # correct one
        # Ensure S is at least 2D
        S = np.atleast_2d(S)
        # print(S)

        # Compute the average asset prices for each path
        avg_asset_prices = np.mean(S, axis=1)  # Average over assets for each path
        # print(avg_asset_prices)

        # Calculate payoff based on the terminal prices (last time step)
        payoff = np.maximum(avg_asset_prices - K, 0)  # Payoff of each path
    ...
    def calculate_delta(self, X_pred, time_array, K, r, sigma, T, D, epsilon=1e-4, mc_paths=20000):
        rows, cols, num_assets = X_pred.shape
        deltas = np.zeros((rows, cols, num_assets))  # Initialize array for deltas

        base_option_prices = self.calculate_call_option_prices(X_pred, time_array, K, r, sigma, T, D, mc_paths)

        for asset_index in range(num_assets):
            # Bump the asset price
            bumped_X_pred = np.copy(X_pred)
            bumped_X_pred[:, :, asset_index] *= (1 + epsilon)  # Increase the price by a small percentage

            # Recalculate the option prices with the bumped asset prices
            bumped_option_prices = self.calculate_call_option_prices(bumped_X_pred, time_array, K, r, sigma, T, D, mc_paths)

            # Calculate the delta
            deltas[:, :, asset_index] = (bumped_option_prices - base_option_prices) / (X_pred[:, :, asset_index] * epsilon)

        # Since we don't need intermediate steps, let's return the delta at t = 0
        return deltas[:, 0, :]
                        ...
                        for optimizer in self.optimizers:
                            ...
                            for mode in tqdm(self.modes, desc="Network Modes", leave=False):
                                for activation in tqdm(self.activations, desc="Activation Functions", leave=False):
                                    ...
                                    model = CallOption(Xi, self.T, M, initial_n_iter, D, int(self.N ** (1 / 5)), layers, mode, activation)
                                    ...
                                    # new_metrics = plotter.plot_training_loss((model.iteration, model.training_loss), D, mode,
...
if __name__ == "__main__":
    ...
    # correlation_types = ["no_correlation", "random_correlation", "restricted_random_correlation"]
    correlation_types = ["random_correlation"]

    save_path = r'C:/Users/aa04947/OneDrive - APG/Desktop/dnnpde_output/exp3'

    initial_lrs = [1e-3]  # Different initial learning rates
    fine_tuning_lrs = [1e-5]  # Different fine-tuning learning rates
    initial_iters = [1000]  # Different initial iterations
    fine_tuning_iters = [500]  # Different fine-tuning iterations

    # initial_lrs = [1e-3, 1e-2, 2e-3, 5e-4, 1e-1, 1e-3, 3e-3]  # Different initial learning rates
    # fine_tuning_lrs = [1e-5, 1e-4, 2e-5, 5e-6, 1e-3, 1e-5, 3e-5]  # Different fine-tuning learning rates
    # initial_iters = [5000, 6000, 7000, 9000, 12000, 20000, 20000]  # Different initial iterations
    # fine_tuning_iters = [1000, 1100, 1400, 1800, 2400, 5000, 5000]  # Different fine-tuning iterations
    ...
    # executor = TrainingExecutor2(Xi, T, Ms, N, Ds, layers_template, modes, activations, optimizers, save_path,
