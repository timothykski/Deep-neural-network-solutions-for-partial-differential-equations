...
class FBSNN(ABC):
    ...
    # def fetch_minibatch(self): # With MLMC
    ...
    #
    #     # Define the levels of MLMC
    #     num_levels = 3  # Number of levels, can be adjusted
    #     M_const = 2  # M constant for MLMC, defines the geometric progression
    #
    #     t_final = np.zeros((M, N + 1, 1))
    #     W_final = np.zeros((M, N + 1, D))
    #
    #     for l in range(num_levels):
    #         # Compute step size and number of time steps for this level
    #         h_l = T / (N * (M_const ** l))
    #         N_l = int(N * (M_const ** l))
    #
    #         # Reduce number of samples at finer levels
    #         M_l = M // (M_const ** l)
    #
    #         # Initialize time steps and Brownian increments
    #         Dt = np.zeros((M_l, N_l + 1, 1))  # Time steps for each trajectory and time snapshot
    #         DW = np.zeros((M_l, N_l + 1, D))  # Brownian increments for each trajectory, time snapshot, and dimension
    #
    #         # Assign time steps
    #         Dt[:, 1:, :] = h_l
    #
    #         # Generate Brownian increments
    #         DW_uncorrelated = np.sqrt(h_l) * np.random.normal(size=(M_l, N_l, D))
    #
    #         # Apply correlation using Cholesky decomposition
    #         L = np.linalg.cholesky(self.correlation_matrix)
    #         DW[:, 1:, :] = np.einsum('ij,mnj->mni', L, DW_uncorrelated)
    #
    #         # Cumulatively sum the time steps and Brownian increments
    #         t = np.cumsum(Dt, axis=1)
    #         W = np.cumsum(DW, axis=1)
    #
    #         # Combine using interpolation for alignment across levels
    #         t_interp = np.linspace(0, T, N + 1)
    #         for m in range(M_l):
    #             interp_func_t = interp1d(t[m, :, 0], t[m, :, 0], kind='linear', fill_value="extrapolate")
    #             interp_func_W = interp1d(t[m, :, 0], W[m, :, :], axis=0, kind='linear', fill_value="extrapolate")
    #             t_resampled = interp_func_t(t_interp).reshape(-1, 1)
    #             W_resampled = interp_func_W(t_interp)
    #
    #             t_final[:M_l] += (1 / (l + 1)) * t_resampled
    #             W_final[:M_l] += (1 / (l + 1)) * W_resampled
    #
    #     # Convert the numpy arrays to PyTorch tensors and transfer them to the configured device (CPU or GPU)
    #     t_final = torch.from_numpy(t_final).float().to(self.device)
    #     W_final = torch.from_numpy(W_final).float().to(self.device)
    #
    #     return t_final, W_final

    def fetch_minibatch(self):  # Generate time + a Brownian motion (no MLMC)
...
class TrainingPlot:
    ...
    def plot_training_loss(self, graph, D, M, mode, activation, optimizer):
        plt.figure(figsize=self.figsize(1.0))
        plt.plot(graph[0], graph[1])
        plt.xlabel('Iterations')
        plt.ylabel('Value')
        plt.yscale("log")
        plt.title(f'Training Loss (D={D}, M={M}, {mode}-{activation}-{optimizer})')
        filename = f'CallOption_{D}D_{M}M_Loss_{mode}_{activation}_{optimizer}.png'
        plt.savefig(os.path.join(self.save_path, filename))
        plt.close()
        # plt.show()

    def plot_prediction(self, t_test, Y_pred, D, M, model, optimizer):
        ...
        plt.title(f'Prediction (D={D}, M={M}, {model.mode}-{model.activation}-{optimizer})')
        plt.legend()
        filename = f'CallOption_{D}D_{M}M_Prediction_{model.mode}_{model.activation}_{optimizer}.png'
        plt.savefig(os.path.join(self.save_path, filename))
        plt.close()
        # plt.show()

    def plot_exact_vs_learned(self, t_test, Y_pred, Y_test, D, M, model, optimizer):
        plt.figure(figsize=self.figsize(1.0))
        ...
        plt.title(f'Exact vs Learned (D={D}, M={M}, {model.mode}-{model.activation}-{optimizer})')
        plt.legend()
        plt.xlabel('$t$')
        plt.ylabel('$Y_t = u(t,X_t)$')
        filename = f'CallOption_{D}D_{M}M_Exact_vs_Learned_{model.mode}_{model.activation}_{optimizer}.png'
        plt.savefig(os.path.join(self.save_path, filename))
        plt.close()
        # plt.show()
...
class StabilityCheck:
    ...
    def plot_stability(self, stability_errors_dict, D, M, optimizer):
        ...
        plt.title(f'{D}-dimensional Call Option Stability (M={M})\nOptimizer: {optimizer}')
        plt.legend()
        filename = f'Stability_{D}D_{M}M_{optimizer}.png'
        plt.savefig(os.path.join(self.save_path, filename))
        plt.close()
        # plt.show()
    ...
    def plot_spectral_radius(self, spectral_radius_dict, D, M, optimizer):
        ...
        plt.title(f'{D}-dimensional Spectral Radius Distribution (M={M})\nOptimizer: {optimizer}')
        plt.axvline(x=1, color='r', linestyle='--', label='Stability Threshold')
        plt.legend(loc='upper right', bbox_to_anchor=(1.1, 1))
        filename = f'SpectralRadius_{D}D_{M}M_{optimizer}.png'
        plt.savefig(os.path.join(self.save_path, filename))
        plt.close()
        # plt.show()
        ...
        plt.xlabel('Spectral Radius')
        plt.ylabel('Density (%)')
        plt.title(f'{D}-dimensional Spectral Radius (PDF) (M={M})\nOptimizer: {optimizer}')
        plt.axvline(x=1, color='r', linestyle='--', label='Stability Threshold')
        plt.legend(loc='upper right', bbox_to_anchor=(1.1, 1))
        filename = f'SpectralRadius_{D}D_{M}M_PDF_{optimizer}.png'
        plt.savefig(os.path.join(self.save_path, filename))
        plt.close()
        # plt.show()
    ...
    def plot_spherical_surface(self, spectral_radius_dict, D, M, optimizer):
        ...
        for i, (key, spec_radius) in enumerate(spectral_radius_dict.items()):
            ...
            ax.plot_trisurf(x_sph, y_sph, z_sph, color=colors[i], alpha=0.6, label=key, linewidth=0.2)

        ax.set_xlabel('X')
        ax.set_ylabel('Y')
        ax.set_zlabel('Spectral Radius')
        ax.set_title(f'Spectral Radius Surface (D={D}, M={M})\nOptimizer: {optimizer}')
        plt.legend(loc='upper right', bbox_to_anchor=(1.1, 1))
        filename = f'SpectralRadiusSurface_{D}D_{M}M_{optimizer}.png'
        plt.savefig(os.path.join(self.save_path, filename))
        plt.show()
    ...
    def plot_jacobian_3d(self, jacobian_dict, D, M, optimizer):
        ...
        for idx, (key, jacobians) in enumerate(jacobian_dict.items()):
            ...
            ax.set_title(f'Jacobian 3D (D={D}, M={M})\nOptimizer: {optimizer}')

            fig.colorbar(surf, ax=ax, shrink=0.5, aspect=5)

        plt.tight_layout()
        plt.savefig(os.path.join(self.save_path, f'Jacobian3D_{D}D_{M}M_{optimizer}.png'), dpi=300)
        plt.close()
        # plt.show()
                ...
                for optimizer in self.optimizers:
                    ...
                    for mode in tqdm(self.modes, desc="Network Modes", leave=False):
                        for activation in tqdm(self.activations, desc="Activation Functions", leave=False):
                            ...
                            #todo: fill in the newest sensitivity analyzed results here
                            initial_lr = 1e-3  # Example value; replace with optimal value after sensitivity analysis
                            fine_tuning_lr = 1e-5  # Example value; replace with optimal value after sensitivity analysis
                            initial_n_iter = 1500  # Example value; replace with optimal value after sensitivity analysis
                            fine_tuning_n_iter = 375  # Example value; replace with optimal value after sensitivity analysis
                            ...
                            plotter.plot_training_loss((model.iteration, model.training_loss), D, M, mode, activation,
                                                       optimizer)
                            plotter.plot_exact_vs_learned(t_test, Y_pred, Y_test, D, M, model, optimizer)
                            ...
                            self.results_df = pd.concat([self.results_df, pd.DataFrame([new_row])],
                                                        ignore_index=True)
                    stability_checker.plot_stability(stability_errors_dict, D, M, optimizer)
                    stability_checker.plot_spectral_radius(spectral_radius_dict, D, M, optimizer)
                    stability_checker.plot_spherical_surface(spectral_radius_dict, D, M, optimizer)
                    stability_checker.plot_jacobian_3d(jacobian_dict, D, M, optimizer)
...
if __name__ == "__main__":
    Ms = [1, 2, 5, 8, 16, 32, 64, 100, 128, 256, 1000]  # Different batch sizes / # MC
    # Ms = [128]
    N = 50
